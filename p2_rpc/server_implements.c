...
void inicializar_autos()
{
    for (int i = 0; i < NUM_AUTOS; i++)
    {
        ...
        snprintf(autos[i].tipoUber, sizeof(autos[i].tipoUber), (i % 3 == 0) ? "UberBlack" : (i % 2 == 0) ? "UberXL"
                                                                                                         : "UberPlanet");
        autos[i].tarifa = (strcmp(autos[i].tipoUber, "UberBlack") == 0) ? 25.0 : (strcmp(autos[i].tipoUber, "UberXL") == 0) ? 15.0
                                                                                                                            : 10.0;
        snprintf(autos[i].placa, sizeof(autos[i].placa), "%03dABC", i);
        autos[i].ganancias = 0.0;
        printf("Auto %d: tipo=%s, placa=%s, pos=(%d, %d), tarifa=%.2f\n",
               i, autos[i].tipoUber, autos[i].placa, autos[i].pos.x, autos[i].pos.y, autos[i].tarifa);
    }
}
...
InfoAuto *solicitarviaje_1_svc(Posicion *pos, struct svc_req *req)
{
    ...
    printf("Solicitud de viaje en (%d, %d)\n", pos->x, pos->y);
